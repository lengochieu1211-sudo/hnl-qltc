...
for old, new, label in [
    ("${stockSummary.map((item, idx) => `", "${sortedStockSummary.map((item, idx) => `", 'warehouse rendering'),
    ("${workVolumes.map((wv, idx) => `", "${sortedWorkVolumes.map((wv, idx) => `", 'work volume rendering'),
    ("${filteredRooms.map(r => {", "${sortedRooms.map(r => {", 'room rendering'),
    ("${filteredChecklist.map(c => `", "${sortedChecklist.map(c => `", 'checklist rendering'),
]:
    rep(old, new, label)

if "const fpDefects = defectsWithDisplayCode.filter(d => d.floorName === fp.floorName || d.floorId === fp.id);" in s:
    s = s.replace(
        "const fpDefects = defectsWithDisplayCode.filter(d => d.floorName === fp.floorName || d.floorId === fp.id);",
        "const fpDefects = defectsForReport.filter(d => d.floorName === fp.floorName || d.floorId === fp.id);"
    )
...
rep(
    ...
    'defect photo appendix follows defect sorting',
)

anchor = "          {/* Defect list grouping/sorting - display-only; map marker placement is intentionally unchanged. */}\n"
if anchor not in s:
    raise SystemExit('sorting UI anchor not found')

ui = '''          {/* Sorting for each report section. Photo appendices intentionally inherit their parent table order. */}
...
required = [
    'warehouseReportSortMode',
    'workVolumeReportSortMode',
    'floorPlanReportSortMode',
    'roomReportSortMode',
    'crewReportSortMode',
    'checklistReportSortMode',
    'sortedStockSummary.map',
    'sortedWorkVolumes.map',
    'sortedRooms.map',
    'sortedChecklist.map',
    'const defectsWithImages = defectsForReport.filter',
    'Nhật ký nhân công + phụ lục ảnh',
]
...
print('PDF section sorting patch complete')
