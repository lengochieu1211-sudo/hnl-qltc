...
assert(webShell.includes('Tag = "toolbar-icon"') && webShell.includes('toolbarTag.StartsWith("toolbar-icon"') && webShell.includes('new Size(30, 30)') && webShell.includes('new Size(28, 24)') && webShell.includes('Color border = toolbarIcon') && webShell.includes('pevent.Graphics.Clear(parentBack)') && webShell.includes('if (!toolbarIcon)') && launcher.includes('tag == "toolbar-icon-success"') && launcher.includes('tag == "toolbar-icon-warning"'), 'all four native actions share one flat browser-style icon surface while sync state changes icon color only');
...
assert(webShell.includes('ShowSyncMenu') && webShell.includes('syncButton = MakeToolbarButton(string.Empty, 30)') && webShell.includes('syncButton.Text = string.Empty') && webShell.includes('toolbar-icon-success') && webShell.includes('toolbar-icon-warning') && webShell.includes('ToolbarGlyph.SyncOk') && webShell.includes('ToolbarGlyph.SyncWarning') && webShell.includes('Đồng bộ ngay') && webShell.includes('Xem chi tiết'), 'native toolbar keeps sync icon-only in the same browser-style surface as reload/menu/collapse while status detail stays in tooltip/menu');
...
assert(releaseTag === '6.3.0-rc2.2.26.23', 'desktop release tag identifies the RC2.2.26.23 offline cold-start + frameless header logo runtime candidate');
