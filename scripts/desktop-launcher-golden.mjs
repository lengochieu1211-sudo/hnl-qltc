...
assert(webShell.includes('ShowSyncMenu') && webShell.includes('MakeToolbarPrimaryButton("Đồng bộ", 108)') && webShell.includes('ToolbarGlyph.SyncOk') && webShell.includes('ToolbarGlyph.SyncWarning') && webShell.includes('Đồng bộ ngay') && webShell.includes('Xem chi tiết'), 'native toolbar keeps one readable sync status button and draws status glyphs without fragile Unicode button text');
...
assert(webShell.includes('Rectangle reloadArc') && webShell.includes('graphics.FillPolygon(brush') && webShell.includes('new Point(right - 7, top + 2)') && webShell.includes('new Point(right - 2, top + 7)'), 'reload glyph has a DPI-safe filled arrowhead and cannot degrade into a C-shaped arc');
...
assert(releaseTag === '6.3.0-rc2.2.26.11', 'desktop release tag identifies the RC2.2.26.11 DPI-safe reload glyph candidate');
