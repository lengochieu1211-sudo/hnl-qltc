...
assert(webShell.includes('ShowSyncMenu') && webShell.includes('MakeToolbarPrimaryButton("Đồng bộ", 100)') && webShell.includes('ToolbarGlyph.SyncOk') && webShell.includes('ToolbarGlyph.SyncWarning') && webShell.includes('Đồng bộ ngay') && webShell.includes('Xem chi tiết'), 'native toolbar keeps one compact sync status button and draws status glyphs without fragile Unicode button text');
...
assert(releaseTag === '6.3.0-rc2.2.26.6', 'desktop release tag identifies the RC2.2.26.6 warehouse Excel undefined-field hotfix candidate');
