...
const runtimeTaskbarSizes = [16, 20, 24, 32, 40, 48];
...
for (const size of runtimeTaskbarSizes) {
  const icon = readPngSize(`public/hnl-taskbar-pixel-${size}.png`);
  assert(icon.width === size && icon.height === size, `browser app-mode has exact pixel-hinted ${size}x${size} HNL icon`);
  assert(indexHtml.includes(`sizes="${size}x${size}" href="/hnl-taskbar-pixel-${size}.png?v=20260909-pixel2"`), `HTML advertises exact ${size}x${size} runtime icon`);
}
assert(indexHtml.includes('/icon-3d-192.png?v=20260909-pixel2') && indexHtml.includes('/icon-3d-512.png?v=20260909-pixel2'), 'running browser app-mode retains HQ install-icon fallbacks');
...
for (const size of runtimeTaskbarSizes) {
  assert(manifestIconSizes.has(`/hnl-taskbar-pixel-${size}.png?v=20260909-pixel2|${size}x${size}|image/png`), `PWA manifest includes exact ${size}x${size} runtime PNG`);
}
assert(manifestIconSizes.has('/icon-3d-192.png?v=20260909-pixel2|192x192|image/png'), 'PWA manifest retains 192x192 HNL PNG fallback');
assert(manifestIconSizes.has('/icon-3d-512.png?v=20260909-pixel2|512x512|image/png'), 'PWA manifest retains 512x512 HNL PNG fallback');
...
assert(iconGolden.includes('hnl-taskbar-pixel-') && iconGolden.includes('$runtimeSizes = @(16,20,24,32,40,48)'), 'Windows golden validates the exact browser runtime pixel frames');
assert(iconGolden.includes('runtime-pixel-contact-sheet.png'), 'Windows golden emits dedicated runtime pixel contact-sheet evidence');
