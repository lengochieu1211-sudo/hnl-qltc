...
assert(webShell.includes('Tag = "toolbar-icon"') && webShell.includes('new Size(30, 30)') && webShell.includes('new Size(28, 24)') && webShell.includes('Color border = toolbarIcon') && launcher.includes('tag == "toolbar-icon"'), 'native utility actions are flat borderless toolbar icons at rest with compact hover-only surfaces');
...
assert(webShell.includes('ShowSyncMenu') && webShell.includes('MakeToolbarPrimaryButton("Đồng bộ", 92)') && webShell.includes('ToolbarGlyph.SyncOk') && webShell.includes('ToolbarGlyph.SyncWarning') && webShell.includes('Đồng bộ ngay') && webShell.includes('Xem chi tiết'), 'native toolbar keeps one compact sync status button and draws status glyphs without fragile Unicode button text');
...
assert(releaseTag === '6.3.0-rc2.2.26.7', 'desktop release tag identifies the RC2.2.26.7 flat compact toolbar + visible download destination candidate');
