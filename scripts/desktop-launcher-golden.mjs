...
assert(webShell.includes('brandLogo.Visible = false') && webShell.includes('brandLabel.Visible = false') && webShell.includes('releaseLabel.Visible = false') && webShell.includes('HNL QLTC • ' + Program.GetReleaseTag()) && webShell.includes('NormalHeaderHeight = 38F'), 'native shell removes duplicate logo/title/version branding from the command strip while retaining version in More');
