...
const checks = [
  ...
  ['Warehouse export keeps WorkVolume reference-only', excel.includes("book_append_sheet(wb, wsWorkVolumes, 'Hạng Mục Thi Công (Chỉ xem)')")],
  ...
  ['Warehouse combined import cannot mutate WorkVolume', warehouse.includes('Hạng mục/Khối lượng has one authoritative editing surface') && !warehouse.includes('onImportWorkVolumes(newWorkVolumes)')],
  ...
  ['WorkVolume dedicated import preserves floor IDs and due date', workTab.includes("row['__floorIds']") && workTab.includes("row['Ngày Hạn Định']")],
