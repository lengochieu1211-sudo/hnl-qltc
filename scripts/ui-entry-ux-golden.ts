...
assert(homeDashboardUi.includes('không hiển thị số giả'), 'Home must never invent metrics for projects whose business data is not loaded');
...
assert(homeDashboardUi.includes('Chia sẻ báo cáo'), 'Trang chủ manpower report must expose quick report sharing');
...
assert(crewUi.includes('Chia sẻ báo cáo quân số · 1 ngày / nhiều ngày · nội dung / ảnh'), 'Crew screen must expose the consolidated share-report entry');
