...
export function buildHealthCenterCopyText(input: HealthCenterExportInput): string {
  ...
  const lines = [
    ...
    `Mặt bằng ảnh: total ${String(floorPlanDiagnostics.total ?? '—')} | pending ${String(floorPlanDiagnostics.pending ?? '—')} | offline ${String(floorPlanDiagnostics.offlineReady ?? '—')}/${String(floorPlanDiagnostics.offlineEligible ?? '—')} | cache ${String(floorPlanDiagnostics.cacheCount ?? '—')} phiên bản / ${String(floorPlanDiagnostics.cacheBytes ?? '—')} bytes | outbox ${String(floorPlanDiagnostics.outboxCount ?? '—')} | outbox bytes ${String(floorPlanDiagnostics.outboxBytes ?? '—')}`,
