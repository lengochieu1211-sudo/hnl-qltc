...
import { subscribeConfirm, type ConfirmRequest } from '../utils/confirmAsync';
import { AlertCircle } from 'lucide-react';

export const GlobalConfirmModal: React.FC = () => {
  const [confirmData, setConfirmData] = useState<ConfirmRequest | null>(null);
    ...
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="p-5 flex flex-col items-center text-center">
          ...
          <h3 className="text-lg font-bold text-slate-900 mb-2">{confirmData.title || 'Xác nhận'}</h3>
          ...
          <div className="flex items-center gap-3 w-full">
            ...
            >
              {confirmData.confirmLabel || 'Đồng ý'}
            </button>
          </div>
        </div>
      </div>
