...
4. Reapply these required production patches if the new ZIP overwrote them:
   ...
   - `src/lib/firebase.ts` must read Firebase config from `VITE_FIREBASE_*`, not from committed JSON keys.
...
7. Commit and push to `main`; Firebase App Hosting should automatically roll out the new version.
